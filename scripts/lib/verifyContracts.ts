...
export const differs = (a: unknown, b: unknown, epsilon = 1e-6): boolean => {
  const na = asNumber(a)
  const nb = asNumber(b)
...
const defaultProbeLow = (meta: ContractParamMetadata): unknown => {
...
const defaultProbeHigh = (meta: ContractParamMetadata): unknown => {
...
export const outOfRangeLow = (min: number): number => {
  return min - Math.max(1, Math.abs(min) + 1)
}

export const outOfRangeHigh = (max: number): number => {
  return max + Math.max(1, Math.abs(max) + 1)
}

const severityForReference = (ref: ContractReference): 'error' | 'warning' => {
...
const hasSafeModeControl = (profile: Profile): boolean => {
...
const hasReducedMotionControl = (profile: Profile): boolean => {
  return (profile.ui?.controls ?? []).some((c) => {
    const id = String(c.id ?? '').toLowerCase()
    const target = String(c.target ?? '').toLowerCase()
    return (
...
const uniqueParamKey = (kind: 'video' | 'audio', node: string, param: string): string => {
  return `${kind}:${node}:${param}`
}

const normalizedNodeName = (node: unknown): string => {
  return typeof node === 'string' ? node.toLowerCase() : ''
}

const validateReference = (
  reference: ContractReference,
  videoLookup: ReturnType<typeof buildVideoNodeLookup>,
  audioLookup: ReturnType<typeof buildAudioNodeLookup>,
  issues: ContractIssue[],
  missingNodes: ContractIssue[],
  missingParams: ContractIssue[],
): void => {
  if (reference.referenceType === 'profile_reactive_target') return
  const lookup = reference.kind === 'video' ? videoLookup : audioLookup
  const nodeDef = lookup.get(reference.node)
  if (!nodeDef) {
    const issue: ContractIssue = {
      severity: severityForReference(reference),
      code: 'MISSING_NODE',
      message: `Referenced ${reference.kind} node "${reference.node}" is not implemented`,
      kind: reference.kind,
      node: reference.node,
  ...
  if (!reference.param || nodeDef.params[reference.param]) return
  const issue: ContractIssue = {
    severity: severityForReference(reference),
    code: 'MISSING_PARAM',
    message: `Referenced ${reference.kind} param "${reference.param}" is not defined for node "${nodeDef.node}"`,
    kind: reference.kind,
    node: nodeDef.node,
...
export const verifyContracts = (rootDir: string): ContractVerificationReport => {
  ...
  for (const reference of loaded.references)
    validateReference(reference, videoLookup, audioLookup, issues, missingNodes, missingParams)

  const validateReactiveTargets = (): void => {
    for (const p of loaded.profiles) {
      ...
      for (let i = 0; i < reactive.length; i++) {
        ...
        if (resolved.kind === 'video') {
          ...
          const nodeName = normalizedNodeName(entry?.node)
          ...
          const nodeName = normalizedNodeName(p.profile.audio_stack?.chain?.[chainIndex]?.node)
  ...
  validateReactiveTargets()

  const collectReferencedParams = (): Map<
    string,
    {
      nodeDef: ContractNodeDefinition
      param: string
  ...
  const referencedParams = collectReferencedParams()

  const validateReferencedParamUsage = (): void => {
  ...
  validateReferencedParamUsage()

  const validateProfilePolicies = (): void => {
    for (const p of loaded.profiles) {
      ...
      const audioClampPolicyCheck = (
        ...
        const ok = numericValue <= clampValue + (epsilon ?? 1e-6)
        return {
          profileId: p.profileId,
          policy: 'safe_mode',
          status: ok ? 'ok' : 'error',
          ...
          param: paramName,
          message: ok
            ? `Audio param respects safe clamp "${clampKey}"`
            : `Audio param exceeds safe clamp "${clampKey}" (${numericValue} > ${clampValue})`,
        }
      }

      const validateAudioSafeModeClamps = (): void => {
        for (const chainDef of profile.audio_stack?.chain ?? []) {
          ...
          for (const [paramName, value] of Object.entries(chainDef.params ?? {})) {
            ...
            const policyCheck = audioClampPolicyCheck(
              nodeDef,
              paramName,
              paramMeta.safeModeClampKey,
              clampValue,
              numericValue,
              paramMeta.epsilon,
            )
            policyChecks.push(policyCheck)
