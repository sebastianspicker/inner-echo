...
const generateConditionsCatalogMd = (catalog: Catalog, profiles: Profile[]): string => {
  ...
  for (const profile of profiles) appendConditionDetail(lines, profile, catalogById)

  return lines.join('\n')
}

const generatePresetSchemaJson = (): string => {
...
const generatePresetSchemaMd = (schemaJson: string): string => {
  const required = [
    'id',
    'label',
    'summary',
