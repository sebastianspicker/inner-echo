 ...
 *
 * Verifies:
 * - `src/content/experience/catalog.json` validates against runtime Zod schemas
 * - every `src/content/experience/profiles/*.json` validates and is present in catalog
...
import { catalogSchema, profileSchema, type Profile } from '../../src/domain/experience/schema'
...
function validateProfileGraph(profile: Profile, failures: { count: number }) {
  const nodes = buildVideoNodes(profile, {
    reducedMotion: false,
