...
describe('engine/audio/audioEngine', () => {
  ...
  it('reports when the browser does not provide microphone access', async () => {
    vi.stubGlobal('navigator', {})
    const onMicStatusChange = vi.fn()
    const control = await createInitializedAudioEngine(DEFAULT_AUDIO, { onMicStatusChange })

    control.requestMic()

    expect(onMicStatusChange).toHaveBeenCalledWith(
      'error',
