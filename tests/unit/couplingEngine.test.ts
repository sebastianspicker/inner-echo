...
function zeroAudio(): AudioMetrics {
  return { ...zeroCoreAudio(), ...zeroBandAudio() }
}

function zeroCoreAudio(): AudioMetrics {
  return { rms: 0, centroid: 0, flux: 0 }
}

function zeroBandAudio(): AudioMetrics {
...
function settleCoupling(
  profile: Profile,
  settings: Parameters<typeof createCouplingEngine>[1],
  audio: AudioMetrics,
  video: VideoMetrics,
) {
  const engine = createCouplingEngine(profile, settings)
  let result = engine.step(1 / 60, audio, video, {})
  for (let index = 0; index < 120; index++) result = engine.step(1 / 60, audio, video, {})
  return result
}
...
describe('engine/reactive/couplingEngine', () => {
  ...
  it('mic metrics drive audio overrides even when video metrics are neutral', () => {
    ...
    const quietResult = settleCoupling(profile, settings, quietAudio, neutralVideo)
    ...
    const loudResult = settleCoupling(profile, settings, loudAudio, neutralVideo)
    ...
    for (const key of [
      ...
      expect(loudResult.audio[key]).toBeGreaterThan(quietResult.audio[key] ?? 0)
    }
  })
