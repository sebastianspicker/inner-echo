...
type SignatureUpdater = (signature: Signature, node: VideoStackNodeDef) => void

const SIGNATURE_UPDATERS: Record<string, SignatureUpdater> = {
  gaze_tunnel(signature, node) {
    signature.gaze += num(node, 'amount')
    signature.edge += num(node, 'edge_gain')
  },
  edge_sharpen(signature, node) {
    signature.edge += num(node, 'amount') * 0.8
  },
  somatic_pulse(signature, node) {
    signature.somatic += num(node, 'depth') + num(node, 'tunnel') * 0.45 + num(node, 'blur') * 0.35
  },
  intrusion_burst(signature, node) {
    signature.intrusion +=
      num(node, 'amount') + num(node, 'burst_probability') * 0.12 + num(node, 'zoom') * 0.06
  },
  salience_competition(signature, node) {
    signature.salience +=
      num(node, 'amount') + num(node, 'marker_strength') * 0.12 + num(node, 'shift') * 1.5
  },
  color_grade(signature, node) {
    signature.mute += Math.max(0, -num(node, 'saturation')) + Math.max(0, -num(node, 'contrast'))
    signature.edge += Math.max(0, num(node, 'contrast')) * 0.25
  },
  temporal_smear(signature, node) {
    signature.inertia += num(node, 'feedback') + Math.max(0, num(node, 'decay') - 0.9)
  },
  glass_veil(signature, node) {
    signature.glass +=
      num(node, 'veil') + num(node, 'feedback') + num(node, 'refraction') * 2 + num(node, 'chroma')
  },
  feedback_loop(signature, node) {
    signature.loop += num(node, 'feedback') + Math.max(0, num(node, 'decay') - 0.9)
  },
  grid_hint(signature, node) {
    signature.grid += num(node, 'amount') * 2
  },
  grain(signature, node) {
    signature.texture += num(node, 'amount')
  },
  haze(signature, node) {
    signature.fog += num(node, 'amount')
  },
  soft_blur(signature, node) {
...
function effectiveVideoStack(
  ...
  return profile.video_stack.flatMap((_, builtIndex) => {
...
function visualSignature(profile: Profile, options?: EffectiveStackOptions): Signature {
  ...
  for (const node of effectiveVideoStack(profile, options)) {
    SIGNATURE_UPDATERS[node.node]?.(signature, node)
  }
...
function forEachProfilePair(visitor: (left: Profile, right: Profile) => void): void {
  for (let i = 0; i < conditionProfiles.length; i++) {
    for (let j = i + 1; j < conditionProfiles.length; j++) {
      visitor(conditionProfiles[i], conditionProfiles[j])
    }
  }
}
...
describe('condition profile distinctiveness acceptance gate', () => {
  ...
  it(
    'proves mic coupling reaches condition-specific visual and audio mechanisms',
    verifyMicCoupling,
  )
})

function verifyMicCoupling() {
  expectCouplingIncrease(panic as Profile, 'video.somatic_pulse.depth')
  expectCouplingIncrease(panic as Profile, 'audio.pulse_tone.mix')
  expectCouplingIncrease(adhd as Profile, 'video.salience_competition.amount')
  expectCouplingIncrease(dpdr as Profile, 'video.glass_veil.veil')
}

function expectCouplingIncrease(profile: Profile, target: string) {
  const quiet = settleCoupling(profile, quietAudio())
  const loud = settleCoupling(profile, loudMicAudio())
  const key = resolvedKey(profile, target)
  const values = target.startsWith('video.')
    ? [loud.video[key], quiet.video[key]]
    : [loud.audio[key], quiet.audio[key]]
