...
function expectComposedProfile(
  result: Awaited<ReturnType<typeof composeEffectiveProfileCore>>,
  expectedStackLength: number | 'nonempty',
): void {
  ...
  if (expectedStackLength === 'nonempty') {
    expect(result.profile.video_stack.length).toBeGreaterThan(0)
  } else {
    expect(result.profile.video_stack).toHaveLength(expectedStackLength)
  }
...
describe('composer/composeCore', () => {
  it('composeEffectiveProfileCore with a single preset produces valid profile', async () => {
    ...
    expectComposedProfile(result, 'nonempty')
  ...
  it('composeEffectiveProfileCore with empty presets and dimensions returns fallback', async () => {
    ...
    expectComposedProfile(result, 0)
  })
  ...
  describe('multimorbid mode (two presets with different weights)', () => {
    ...
    function composeEvenlyWeightedProfiles() {
    ...
    it('includes nodes unique to one preset', async () => {
      const result = await composeEvenlyWeightedProfiles()
