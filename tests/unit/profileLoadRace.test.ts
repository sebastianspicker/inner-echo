...
function makeSources(loadPresetProfile: ComposeSources['loadPresetProfile']): ComposeSources {
  return {
    loadPresetProfile,
...
function expectGrainAmount(profile: Profile, amount: number): void {
  const grain = profile.video_stack.find((node) => node.node === 'grain')
  expect(grain).toBeDefined()
  expect((grain!.params as Record<string, number>).amount).toBeCloseTo(amount, 1)
}
...
describe('profileLoadRace: concurrent compose calls', () => {
  it('concurrent composeEffectiveProfileCore calls produce independent results', async () => {
    ...
    const sources = makeSources(async (profileId: string) => {
  ...
  it('useAsyncEffect cancellation pattern: late resolver is ignored', async () => {
    ...
    expectGrainAmount(activeResult!, 0.8)
  })
  ...
  it('compose with missing preset does not affect concurrent call for valid preset', async () => {
    ...
    expectGrainAmount(validResult.profile, 0.7)
  })
})
