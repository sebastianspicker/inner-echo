...
describe('engine/effects/paramUtils – resolveBooleanParam', () => {
  ...
  it('falls through to an unkeyed boolean when the indexed value has the wrong type', () => {
    const params = makeParams({ controlValues: { '0.enabled': 1, enabled: false } })
    expect(resolveBooleanParam(params, 'enabled', true)).toBe(false)
  })
...
describe('engine/effects/paramUtils – resolveNumberParam', () => {
  ...
  it('falls through to an unkeyed number when the indexed value has the wrong type', () => {
    const params = makeParams({ controlValues: { '0.amount': false, amount: 0.7 } })
    expect(resolveNumberParam(params, 'amount', 0)).toBe(0.7)
  })
})
