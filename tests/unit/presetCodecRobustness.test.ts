...
describe('presetCodecRobustness: Base64Url edge cases', () => {
  for (const { conditionId, scenario } of [
    { conditionId: 'x', scenario: 'round-trips a one-character condition id' },
    {
      conditionId: 'abc',
      scenario: 'round-trips a three-character condition id without padding',
    },
  ]) {
    it(scenario, () => {
      const hash = encodePresetToHash(makePayload({ conditionId }))
      const decoded = decodePresetFromHash(hash)
      expect(decoded.ok).toBe(true)
      if (decoded.ok) expect(decoded.payload.conditionId).toBe(conditionId)
    })
  }
