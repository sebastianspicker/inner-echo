...
const mockCatalog: CatalogEntry[] = [
  ...
  { id: 'tinnitus', label: 'Tinnitus', description: 'Phantom auditory perception', recommended: true },
...
describe('ui/ConditionPicker', () => {
  it('renders all catalog entries as options', () => {
    render(
      <ConditionPicker catalog={mockCatalog} value="dpdr" onChange={vi.fn()} />,
    )
  ...
  it('marks recommended entries with "(empfohlen)" suffix', () => {
    render(
      <ConditionPicker catalog={mockCatalog} value="dpdr" onChange={vi.fn()} />,
    )
    const options = screen.getAllByRole('option')
    const tinnitus = options.find((o) => o.textContent?.includes('Tinnitus'))
    expect(tinnitus?.textContent).toBe('Tinnitus (empfohlen)')
  })
  ...
  it('does not show description when selected entry has none', () => {
    render(
      <ConditionPicker catalog={mockCatalog} value="migraine" onChange={vi.fn()} />,
    )
  ...
  it('calls onChange with the selected option value', () => {
    ...
    render(
      <ConditionPicker catalog={mockCatalog} value="dpdr" onChange={onChange} />,
    )
    const select = screen.getByRole('combobox', { name: /condition/i })
  ...
  it('disables select when disabled prop is true', () => {
    render(
      <ConditionPicker catalog={mockCatalog} value="dpdr" onChange={vi.fn()} disabled />,
    )
    const select = screen.getByRole('combobox', { name: /condition/i })
  ...
  it('renders empty select when catalog is null', () => {
    render(
      <ConditionPicker catalog={null} value="" onChange={vi.fn()} />,
    )
  ...
  it('renders empty select when catalog is empty array', () => {
    render(
      <ConditionPicker catalog={[]} value="" onChange={vi.fn()} />,
    )
    const options = screen.queryAllByRole('option')
    expect(options).toHaveLength(0)
  })
