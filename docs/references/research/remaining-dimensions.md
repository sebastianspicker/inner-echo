# Research notes for remaining experience dimensions
...
Evidence is consistent with intrusions behaving like salient, involuntary “insertions” into the current perceptual or cognitive stream, often with cue-triggering and sensory vividness. A cautious AV metaphor is therefore a brief, non-graphic interruption to the current audiovisual field. It can enter and then dissipate without implying any literal content.
...
- PTQ validation emphasizes core loop-like features: repetitiveness, intrusiveness, and disengagement difficulty. These inform the loop mechanics used in the design.
...
- Counterevidence and nuance: some work suggests enhanced saliency representations for gains and losses in valuation circuitry, underscoring heterogeneity. “Numbing” is not a single uniform mechanism (primary).
...
## Motif consistency check
Recommended
- Mild desaturation + mild lowpass + gentle compression (a restrained representation of muted affect without distortion).

Optional
- Soft blur (can drift toward “cognitive fog” if overdone, so keep it low).
...
- A restrained metaphor uses reduced agency or ownership and increased self-observation, not distortion.
...
## Mapping hypothesis (video + audio motifs + safety)
### Video motifs (metaphor hypotheses)
- User-controlled micro-latency in non-critical animations (clamped): a tiny delay between input and visual confirmation can metaphorically suggest reduced agency. It must be optional and must not impair usability.
...
- Reduced Motion: keep effects static. If the input-latency metaphor risks frustration, replace it with gentler grading.
...
- Edge sharpen (very subtle, optional): can hint at “too sharp/too much detail,” but risks irritation and requires caution.
...
## Motif consistency check
Recommended
- Layer density increase + strong limiter + quick calm toggle (represents “too much at once” while limiting triggering output).
...
### Video motifs (metaphor hypotheses)
- Focus jitter (low..medium, high smoothing): represents unstable focusing without nausea-inducing motion.
...
## Motif consistency check
Recommended
- High-smoothing focus jitter + strong limiter (a restrained representation of instability without nausea).
...
- Grid hint (very low): hints at “checking/ordering” structure. Keep it subtle to avoid stereotype.
...
## Motif consistency check
Recommended
- Low feedback loop + low-mix short delay + limiter (bounded representation with no escalation).
...
Source-verification note: Core peer-reviewed sources supporting the most
constraint-heavy design inferences were checked through publisher and PubMed
